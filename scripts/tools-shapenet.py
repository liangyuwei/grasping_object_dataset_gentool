  ...
  <model name='" + str(object_index) + '_' + object_name + '_' + str(object_scale) + "'>\n\
    ...
    def obtain_models_info(self, mesh_in_path, h5_path, h5_name):
        ...
        for obj_file_name in os.listdir(dataset_path):               
            ...
            if volume < 0.01: # sometimes volume could be really low...
...
#    proc_tools.convert_obj_dae(mesh_in_dir=mesh_path, mesh_out_dir=mesh_path+'-dae')
    proc_tools.convert_dae_obj(mesh_in_dir=mesh_path, mesh_out_dir=mesh_path) # +'-obj') # .obj can stay in the same folder
    ...
    proc_tools.generate_model_folders(start_index=1, mesh_suffix='dae', mesh_in_path=mesh_path)# +'-dae') # already have dae, no need to use -dae folder
    #'/home/liangyuwei/grasping-object-dataset/meshes/chosen_from_the_other_2_folders-dae') #
